...
const useDefaultItemSize = makeResetStyles({
  height: '32px',
...
export const useBreadcrumbItemStyles_unstable = (state: BreadcrumbItemState): BreadcrumbItemState => {
  ...
  const iconResetStyles = useIconResetStyles();
  ...
  if (state.icon) {
    state.icon.className = mergeClasses(iconStyles.base, iconStyles[state.size], iconResetStyles, state.icon.className);
  }
